...
| 宿主 runner | GitHub 的机器，只负责起进程 | 一律 `ubuntu-latest`，**故意不钉** |

宿主为什么反而不能钉：GitHub 会删旧镜像，18.04、20.04 都已经没了，`ubuntu-24.04` 迟早同样下场。钉一个会被删的标签，等于给 workflow 设了个定时炸弹，换来的还是假的确定性 —— 真正的确定性来自容器，而镜像 digest 不会过期。`ubuntu-latest` 是唯一永远存在的 Ubuntu 标签。

而且宿主本来就影响不到产物：`CGO_ENABLED=0` 之下，宿主的 libc、头文件、链接器一概不参与编译。所以「宿主用 latest + 容器钉死」既不会坏，又是永久一致的。

唯一会让容器的发行版变得要紧的情况是**开启 cgo**：那时候 glibc 的符号版本会被写进二进制，用新系统编出来的东西在老系统上跑不起来，这才是「故意钉一个老基础镜像」有意义的场景。本项目没有 cgo，所以 Debian 12 还是 Ubuntu 22 对产物没有任何区别 —— 官方 `golang` 镜像是 Debian 基底，直接用它比自己攒一个 Ubuntu 基底的镜像省事得多。真要换成 Ubuntu 22 基底，得自己写 Dockerfile 装 Go 并校验 tarball，收益为零。

### 不是假设，是验证

`verify` job 在**另一台新机器**上用同一个 digest 重新编一遍，比对二进制的 SHA256，对得上才算数。这道检查兜住的是「有东西从钉死的镜像外面漏进来了」——比如时间戳、路径、宿主状态。

构建本身是可复现的（`-trimpath` / `-buildvcs=false` / `-buildid=` / 归档时间戳归一），同一个 commit 编两次，二进制和归档都逐字节相同。

想要更强的证据，可以再加一条用**不同基底镜像**的腿（比如 `golang:1.27.1-alpine`，musl 而非 glibc），两个完全不同的用户态编出一样的字节，那就把「基底发行版不参与」也证掉了。没默认开是因为写这份代码的环境没有 docker daemon，这条我没能实测。
